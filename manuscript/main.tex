...
Using national staple-crop statistics and exogenous economic indicators, we evaluated Ridge and Random Forest models for predicting U.S. food security. In this section, we evaluate how our models predict U.S. food security in the held-out period and what signals contributing to model estimates. 

Across models' performance, the Random Forest model, trained on $\Delta FSI$, was selected as the primary approach as it provided the most reliable out-of-sample tracking of $FSI$ comparing with alternative models and our persistence baseline. We therefore present the Random Forest first, followed by diagnostics and an auxiliary directional evaluation.
...
Figure \ref{fig:2} compares observed $FSI$ values in the 2013-2023 test period with predictions from the Random Forest model. We set the y-axis to start at 0.60, rather than 0, to focus on variation within the test range and to highlight differences among the model. Overall, the Random Forest model is able to track the broad observed trajectory, including a mid-decade decline and the sharp post-2017 increase. This suggests that the model captures year by year changes rather than only the trend in the long run. The most visible errors occur around the sharper transitions, where the model either over- or underestimates the change magnitude. But, the close alignment in most years shows that crop and “shock/lag” features contain usable signals for national food-security dynamics.
