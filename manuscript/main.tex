...
\footnote{\textit{Word Count (excluding figures, tables, references, and title page):} 2932}
...
Across all model configurations $\Delta FSI$ showed strong yearly persistence, meaning a "no-change" forecast. Figures 2, 3, 4, and Table 1 represent our first step. Figures 5a, 5b, and Table 2 showcase our second step. We translated our predicted $\Delta FSI$ back into levels by reconstructing “predicted” food security as: $FSI_{\text{pred}}(t) = FST(t-1) + \Delta FSI_{\text{pred}}(t)$ for Figures 2 and 3. This allows for better visualization.

The best performing model was Random Foresting, so the majority of our figures present those results, except for model comparison in Table 2.
